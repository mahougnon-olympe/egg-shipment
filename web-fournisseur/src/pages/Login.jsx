...
const KEY = 'form_login_fournisseur';
...
export default function Login({ onLogin }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (user.role !== 'fournisseur') return setError('Accès réservé au fournisseur');
  ...
  return (
    <div style={{ maxWidth: 360, margin: '4rem auto' }}>
      ...
      <p style={{ marginTop: '1rem', textAlign: 'center', fontSize: '.9rem', color: '#8B7355' }}>
        Pas encore de compte ? <Link to="/inscription" style={{ color: '#E0A516', fontWeight: 600 }}>Créer un compte</Link>
      </p>
    </div>
  );
}
